...
import {
    ...
    MessageCircle, XCircle, RefreshCw
} from 'lucide-react';
import {
                ...
                {activeTab === 'seo' && (
                    <div className="space-y-6">
                        ...
                        <div className="bg-white shadow sm:rounded-lg border border-gray-200 p-6">
                                ...
                                <div className="flex gap-2">
                                    <button
                                        onClick={async () => {
                                            ...
                                            try {
                                                ...
                                                // @ts-ignore
                                                setQueueStats(stats);
                                                const sitemap = await import('@/actions/indexing').then(m => m.getSitemapUrls());
                                                setPseoPages(sitemap); // Reuse pseoPages state for sitemap items temporarily or use a new state
                                            } catch (e: any) {
                                    ...
                                    <button
                                        onClick={async () => {
                                            ...
                                            setIsLoading(true);
                                            try {
                                                const res = await import('@/actions/indexing').then(m => m.scheduleFreshUrls(50));
                                                alert(res.message);
                                                // Refresh stats
                                                const stats = await import('@/actions/indexing').then(m => m.getQueueStats());
                                                // @ts-ignore
                                        ...
                                        className="inline-flex items-center px-4 py-2 border border-purple-500 text-purple-600 bg-purple-50 hover:bg-purple-100 rounded-md text-sm font-medium"
                                    >
                                    ...
                                    <button
                                        onClick={async () => {
                                            ...
                                            try {
                                                ...
                                                const res = await import('@/actions/indexing').then(m => m.processQueueBatch(10));
                            ...
                            {/* Stats Grid */}
                            {/* @ts-ignore */}
                            {stats && ( // reusing generic stats or creating new state? Let's use specific queueStats if possible
                                            ...
                                            <th className="px-6 py-3 text-left">
                                                <input
                                                    ...
                                                    onChange={(e) => {
                                                        if (e.target.checked) {
                                                            // @ts-ignore
                                                            setSelectedUrls(pseoPages.map(p => p.url));
                                                        } else {
                                                            setSelectedUrls([]);
                                                        }
                                                    }}
                                                    // @ts-ignore
                                                    checked={pseoPages.length > 0 && selectedUrls.length === pseoPages.length}
